...
from gr00t.experiment.slack_callback import SlackNotificationCallback, GradExplosionGuardCallback, CheckpointEvalCallback
...
def run(config: Config):
    ...
    trainer.add_callback(GradExplosionGuardCallback())

    if config.training.eval_episode_dir:
        trainer.add_callback(CheckpointEvalCallback(
            eval_episode_dir=config.training.eval_episode_dir,
            max_frames=config.training.eval_max_frames,
        ))
